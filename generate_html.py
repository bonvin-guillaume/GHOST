...
<head>
    ...
    <style>
        * {{
        ...
        .miss-band.signal .miss-band-label {{
            background: rgba(56, 189, 248, 0.95);
        }}

        .miss-band.background .miss-band-label {{
        ...
        .miss-marker.signal .miss-marker-label {{
            background: rgba(56, 189, 248, 0.9);
        }}

        .miss-marker.background .miss-marker-label {{
...
<body>
    <div class="container">
        <div class="info-banner">
            ...
            <ul>
                ...
                <li>On a MISS plot: select signal then background — click = one scan angle, drag = average over a range (run <code>python miss_spectrum_server.py</code> first)</li>
            ...
            <div class="viewer-panel" id="viewer-panel">
                <div class="viewers-grid">
                    <div class="viewer-section">
                        <div class="miss-click-status" id="miss-click-status">Next: signal (click or drag a range) — start <code>miss_spectrum_server.py</code></div>
    ...
    <script>
        ...
        // optional onLoaded(img) is called after the image element is inserted.
        function loadViewerImage(viewer, url, alt, onLoaded) {{
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            // Find the link whose timestamp is closest to targetTs (before or after).
            function _findClosest(links, targetTs) {{
                let best = null;
            ...
            // When a MISS plot is shown, update SONY (col 5, cells[4]) to the
            // closest frame in time (before or after the MISS timestamp).
            function syncViewersToMiss(missLink) {{
                const ts = parseInt(missLink.getAttribute('data-timestamp'), 10);
                if (isNaN(ts)) return;

                const row = missLink.closest('tr');
                if (!row) return;
                ...
                if (cells[4]) {{
                    const sonyLinks = Array.from(cells[4].querySelectorAll('a[data-timestamp]'));
                    const closestSony = _findClosest(sonyLinks, ts);
                    if (closestSony) displayFile(closestSony, false);
                }}
            }}

            // sync=true  → user-initiated action: update keyboard index and trigger
            //              cross-viewer sync when a SONY/MISS/GOA file is shown.
            // sync=false → programmatic sync call: skip those side-effects to avoid loops.
            //              Still highlights and scrolls the link into its file list.
            function displayFile(link, sync) {{
                ...
                if (filename.startsWith('MISS-') || filename.startsWith('MISS2-')) {{
                    ...
                    loadViewerImage(missViewer, url, cleanFilename, function(img) {{
                        resetMissClickUi('Next: signal (click or drag a range)');
                        prepareMissOverlay(img);
                    }});
                    
                    // Sync SONY to the closest frame in time
                    if (sync) {{
                        syncViewersToMiss(link);
                    }}
                ...
                // Always scroll so the highlighted filename is visible in its list.
                // Use instant scroll for synced links to avoid fighting the primary smooth scroll.
                link.scrollIntoView({{
                    behavior: sync ? 'smooth' : 'auto',
                    block: 'nearest',
                    inline: 'nearest'
                }});
            }}
            ...
            // --- MISS plot: signal then background (click = row, drag = mean range) ---
            const MISS_CLICK_URL = 'http://127.0.0.1:8765/click';
            const missClickStatus = document.getElementById('miss-click-status');
            let missClickPhase = 'signal'; // 'signal' | 'background'
            const DRAG_THRESHOLD_PX = 6;
            let missDrag = null; // {{ img, startX, startY, startClientX, startClientY }}

            function resetMissClickUi(message) {{
                missClickPhase = 'signal';
                if (missClickStatus) {{
                    missClickStatus.textContent = message || 'Next: signal (click or drag a range)';
                }}
                const markers = document.getElementById('miss-markers');
                if (markers) markers.innerHTML = '';
                const dragRect = document.getElementById('miss-drag-rect');
                if (dragRect) dragRect.style.display = 'none';
            }}

            function prepareMissOverlay(img) {{
                if (!img || img.closest('.miss-viewer-wrap')) return;
                const wrap = document.createElement('div');
                wrap.className = 'miss-viewer-wrap';
                const markers = document.createElement('div');
                markers.id = 'miss-markers';
                markers.className = 'miss-markers';
                const dragRect = document.createElement('div');
                dragRect.id = 'miss-drag-rect';
                dragRect.className = 'miss-drag-rect';
                img.parentNode.insertBefore(wrap, img);
                wrap.appendChild(img);
                wrap.appendChild(markers);
                wrap.appendChild(dragRect);
            }}

            function missFilenameFromImg(img) {{
                let filename = img.alt || 'unknown';
                if (selectedMissLink) {{
                    filename = selectedMissLink.textContent
                        .replace(' 📈', '')
                        .replace(' 📊', '')
                        .trim();
                }}
            ...
            function clientToPng(img, clientX, clientY) {{
            ...
            function addMissPointMarker(img, yPx, role) {{
                const markers = document.getElementById('miss-markers');
                if (!markers || !img.naturalHeight) return;
                const rect = img.getBoundingClientRect();
                const displayY = (yPx / img.naturalHeight) * rect.height;
                const marker = document.createElement('div');
                marker.className = 'miss-marker ' + role;
                marker.style.top = displayY + 'px';
                const label = document.createElement('span');
                label.className = 'miss-marker-label';
                label.textContent = role === 'signal' ? 'signal' : 'background';
                marker.appendChild(label);
                markers.appendChild(marker);
            }}

            function addMissRangeBand(img, y0Px, y1Px, role) {{
                const markers = document.getElementById('miss-markers');
                if (!markers || !img.naturalHeight) return;
                const rect = img.getBoundingClientRect();
                const topPx = Math.min(y0Px, y1Px);
                const botPx = Math.max(y0Px, y1Px);
                const top = (topPx / img.naturalHeight) * rect.height;
                const height = Math.max(2, ((botPx - topPx) / img.naturalHeight) * rect.height);
                const band = document.createElement('div');
                band.className = 'miss-band ' + role;
                band.style.top = top + 'px';
                band.style.height = height + 'px';
                const label = document.createElement('span');
                label.className = 'miss-band-label';
                label.textContent = (role === 'signal' ? 'signal' : 'background') + ' range';
                band.appendChild(label);
                markers.appendChild(band);
            }}

            function updateDragRect(img, x0, y0, x1, y1) {{
                const dragRect = document.getElementById('miss-drag-rect');
                if (!dragRect) return;
            ...
            async function submitMissSelection(payload, visual) {{
                const role = missClickPhase;
                if (role === 'signal') {{
                    const markers = document.getElementById('miss-markers');
                    if (markers) markers.innerHTML = '';
                }}
                if (visual.kind === 'point') {{
                    addMissPointMarker(visual.img, visual.y, role);
                }} else {{
                    addMissRangeBand(visual.img, visual.y0, visual.y1, role);
                }}
                console.log('MISS selection', payload);

                try {{
                    const response = await fetch(MISS_CLICK_URL, {{
                        method: 'POST',
                        headers: {{ 'Content-Type': 'application/json' }},
                        body: JSON.stringify(payload)
                    }});
                    const result = await response.json();
                    console.log('MISS selection server reply', result);

                    if (result.status === 'need_background') {{
                        missClickPhase = 'background';
                        if (missClickStatus) {{
                            missClickStatus.textContent =
                                'Next: background (click or drag a range)';
                        }}
                    }} else if (result.status === 'plotted') {{
                        missClickPhase = 'signal';
                        if (missClickStatus) {{
                            missClickStatus.textContent =
                                'Spectrum plotted — next selection starts a new signal';
                        }}
                    }} else if (!result.ok) {{
                        if (missClickStatus) {{
                            missClickStatus.textContent = 'Selection failed — see console';
                        }}
                    }}
                }} catch (err) {{
                    console.warn(
                        'MISS click server unreachable. Start it with: python miss_spectrum_server.py',
                        err
                    );
                    if (missClickStatus) {{
                        missClickStatus.textContent =
                            'Server unreachable — run: python miss_spectrum_server.py';
                    }}
                    if (missClickPhase === 'signal') {{
                        missClickPhase = 'background';
                    }} else {{
                        missClickPhase = 'signal';
                    }}
            ...
            missViewer.addEventListener('mousedown', function(e) {{
                const img = e.target;
                if (!img || img.tagName !== 'IMG' || !img.naturalWidth) return;
                if (e.button !== 0) return;
                e.preventDefault();
                const pos = clientToPng(img, e.clientX, e.clientY);
                missDrag = {{
                    img: img,
                    startX: pos.x,
                    startY: pos.y,
                    startDisplayX: pos.displayX,
                    startDisplayY: pos.displayY
                }};
            }});

            document.addEventListener('mousemove', function(e) {{
                if (!missDrag) return;
                const img = missDrag.img;
                const pos = clientToPng(img, e.clientX, e.clientY);
                updateDragRect(
                    img,
                    missDrag.startDisplayX,
                    missDrag.startDisplayY,
                    pos.displayX,
                    pos.displayY
                );
            }});
            ...
            document.addEventListener('mouseup', async function(e) {{
                if (!missDrag) return;
                const img = missDrag.img;
                const start = missDrag;
                missDrag = null;
                const dragRect = document.getElementById('miss-drag-rect');
                if (dragRect) dragRect.style.display = 'none';

                const pos = clientToPng(img, e.clientX, e.clientY);
                const dx = pos.x - start.startX;
                const dy = pos.y - start.startY;
                const moved = Math.hypot(dx, dy);
                const filename = missFilenameFromImg(img);
                const role = missClickPhase;
                ...
                if (moved < DRAG_THRESHOLD_PX) {{
                    ...
                    visual = {{ kind: 'point', img: img, y: start.startY }};
                }} else {{
                    payload = {{
                        filename: filename,
                        kind: 'range',
                    ...
                    visual = {{ kind: 'range', img: img, y0: start.startY, y1: pos.y }};
                }}
                await submitMissSelection(payload, visual);
            }});
